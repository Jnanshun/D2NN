...
# preview images and check the split
...
def nonlinear_act(input_):
    phase = torch.angle(input_)
    out = torch.abs(input_)**2
    out = out * torch.exp(-20/(1+out/6e-3))
    out = torch.sqrt(torch.abs(out))*torch.exp(1j*phase)
    return out
...
baseline_eval1.to_csv('./acc_data_nonlinear_5.csv')
baseline_eval2.to_csv('./loss_data_nonlinear_5.csv')
...
torch.save(model,'./nonlinear_5.pt')
D2NN=torch.load('./nonlinear_5.pt')
...
ax0.imshow(phase_mask[1], cmap = plt.get_cmap('RdYlBu'))
ax1.imshow(phase_mask[2], cmap = plt.get_cmap('RdYlBu'))
ax2.imshow(phase_mask[3], cmap = plt.get_cmap('RdYlBu'))
ax3.imshow(phase_mask[4], cmap = plt.get_cmap('RdYlBu'))
ax4.imshow(phase_mask[5], cmap = plt.get_cmap('RdYlBu'))
ax0.axis('off')
ax1.axis('off')
ax2.axis('off')
ax3.axis('off')
